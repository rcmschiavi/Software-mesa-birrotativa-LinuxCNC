# coding: utf-8
'''
Código para testar a rotina de program separado na BBB
    ...
    def EXEC_PROGRAM(self):
        ...
        if self.prg_point < len(self.activeProgram) and not self.controller.isMoving() and not self.doingTask:
            if self.waitForRobot:
                ...
                self.controller.setMachinePos(self.activeProgram[self.prg_point][0],
                                              self.activeProgram[self.prg_point][1],
                                              self.activeProgram[self.prg_point][2])
                self.JPA.TASK_EXEC = 1
                data = self.JPA.STATUS()
                self.qSend.put(2, data)
                self.doingTask = True
